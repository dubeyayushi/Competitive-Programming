...
bool isItSafe(vector<vector<int>>&grid, int i, int j, int n){
    return i>=0 && j>=0 && i<n && i<n && grid[i][j]==-1;
}
...
void f(vector<vector<int>>&grid, int i, int j, int n, int count){
    ...
    // for every position we can go only if it is safe
    for (int k = 0; k<8; k++){
...
void knightsTour(int n, int i, int j){
    vector<vector<int>>grid(n, vector<int>(n, -1));    // -1 denotes the position is vacant
    f(grid, i, j, n, 0);
}
